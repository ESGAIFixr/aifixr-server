...
function MainContent({
    ...
    setStandard: (s: ESGStandard) => void;
    mainTab: MainTab;
    setMainTab: (t: MainTab) => void;
    resultsSubTab: ResultsSubTab;
    setResultsSubTab: (t: ResultsSubTab) => void;
    inputSubTab: InputSubTab;
    setInputSubTab: (t: InputSubTab) => void;
    ...
    return (
        ...
        >
            <div className="flex flex-col h-full">
                {/* Header */}
                <header className="sticky top-0 z-10 flex h-16 items-center gap-2 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-2 sm:px-4 overflow-x-hidden">
                    <SidebarTrigger className="shrink-0 flex-shrink-0" />
                    <div className="flex-1 min-w-0" />
                    {/* Standard Info Badge */}
                    <div className="inline-flex items-center gap-1 sm:gap-1.5 bg-aifix-secondary-light text-aifix-secondary px-2 sm:px-3 py-1.5 sm:py-2 rounded-full border border-aifix-secondary/20 shrink-0 max-w-[calc(100vw-var(--sidebar-width)-4rem)] sm:max-w-none">
                        <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-aifix-secondary rounded-full animate-pulse shrink-0" />
                        <span className="text-[10px] sm:text-xs font-medium whitespace-nowrap overflow-hidden text-ellipsis">
                            <span className="hidden lg:inline">Currently using {standard} standard / </span>
                            현재 {standard} 기준
                        </span>
                    </div>
                </header>
