package site.aifixr.api.oauthservice.naver;
...
import site.aifixr.api.oauthservice.config.NaverConfig;
import site.aifixr.api.oauthservice.naver.NaverService.OAuthUserResponse;
...
public class NaverController {
    ...
    public ResponseEntity<Map<String, String>> getNaverLoginUrl() {
        try {
            ...
            if (naverConfig.getClientId() == null || naverConfig.getClientId().isEmpty()) {
                log.error("NAVER_CLIENT_ID is not configured");
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body(Map.of("error", "Naver OAuth is not properly configured. Please set NAVER_CLIENT_ID."));
            }

            if (naverConfig.getRedirectUri() == null || naverConfig.getRedirectUri().isEmpty()) {
            ...
            log.info("Generating Naver login URL with client_id: {}, redirect_uri: {}",
                    naverConfig.getClientId().substring(0, Math.min(4, naverConfig.getClientId().length())) + "...",
                    naverConfig.getRedirectUri());

            String loginUrl = UriComponentsBuilder
                    .fromUriString(naverConfig.getAuthorizeUri())
    ...
    public ResponseEntity<?> naverCallback(
        ...
        String frontendUrl = "http://localhost:3000/oauth/naver/callback";
